...
test("full checkout journey rehearsal", async () => {
  ...
  assert.strictEqual((await submit.finished).status, 201);
  assert.strictEqual(preJson.quote_version, 1);
  assert.strictEqual(couponJson.quote_version, 2);
});

test("reject expired hold on submit", async () => {
  seedOnHandSku("SKU-E", 2);
  const pre = captureResponse();
  await handle_checkout_preflight(
    mockInboundJson(
      { cart_id: "cart-hold-exp", subtotal: 100, lines: [{ sku: "SKU-E", qty: 1 }] },
      ACME_AUTH,
    ),
    pre.res,
  );
  ...
  const { reserveCartHold } = await import("../services/reservation_ledger");
  reserveCartHold({
    cartId: "cart-hold-exp",
    lines: [{ sku: "SKU-E", qty: 1 }],
    correlationId: "corr-expired",
    organizationId: "org-acme-01",
    quoteVersion: preJson.quote_version,
  ...
  const submitOut = await submit.finished;
  assert.strictEqual(submitOut.status, 422);
  const json = JSON.parse(submitOut.body) as { code: string };
  assert.strictEqual(json.code, "HOLD_EXPIRED");
});
